...
export default function TestPanel({ problem, output, activeTab, setActiveTab, submissions, onSelectSubmission, isDarkMode = true }: TestPanelProps) {
    ...
    const getStatusColor = (status: string) => {
        ...
        if (status === "Running..." || status === "Judging...") return "text-blue-500";
        return "text-zinc-500";
    };

    // Helper to determine if pending
    const isPending = output?.verdict === "Running..." || output?.verdict === "Judging...";
    ...
    const mutedText = isDarkMode ? "text-zinc-500" : "text-slate-400";
    const hoverBg = isDarkMode ? "hover:bg-white/5" : "hover:bg-slate-100";
    const inputBg = isDarkMode ? "bg-[#121212] border-white/5" : "bg-slate-50 border-slate-200";

    return (
        <div className={`flex flex-col h-full font-sans border-t ${bgColor} ${borderColor} ${textColor}`}>
            {/* Header / Tabs */}
            <div className={`flex items-center h-10 border-b px-2 select-none relative z-20 ${bgColor} ${borderColor}`}>
                <button
                    onClick={() => setActiveTab("testcases")}
                    className={`relative flex items-center gap-2 px-4 h-full text-xs font-medium transition-all ${activeTab === "testcases"
                        ? "text-blue-400 bg-blue-500/5"
                        : `${mutedText} hover:text-zinc-300 ${hoverBg}`
                        }`}
                >
                    <Terminal size={14} />
                    Test Cases
                ...
                <button
                    ...
                    className={`relative flex items-center gap-2 px-4 h-full text-xs font-medium transition-all ${activeTab === "results"
                        ? `text-white ${isDarkMode ? 'bg-white/5' : 'bg-slate-100/50 text-slate-900'}`
                        : `${mutedText} hover:text-zinc-300 ${hoverBg}`
                        }`}
                >
                    {output?.verdict === "Accepted" ? (
                        <CheckCircle2 size={14} className="text-green-500" />
                    ) : output?.verdict && !isPending ? (
                        <XCircle size={14} className="text-red-500" />
                    ) : (
                        <Clock size={14} className={isPending ? "animate-spin text-blue-500" : ""} />
                    )}
                    Results
                    {activeTab === "results" && (
                        <div className={`absolute bottom-0 left-0 w-full h-[1px] shadow-[0_0_8px_rgba(255,255,255,0.4)] ${output?.verdict === 'Accepted' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : (output?.verdict && !isPending) ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]' : 'bg-blue-500'}`}></div>
                    )}
                </button>
                <button
            ...
            <div className={`flex-1 overflow-hidden relative ${bgColor}`}>
                ...
                {activeTab === "testcases" && (
                    <div className="h-full flex flex-col">
                        ...
                        <div className={`p-3 border-b flex-shrink-0 ${borderColor} ${bgColor}`}>
                                ...
                                {problem.test_cases?.filter((tc: any) => !tc.is_hidden).map((tc: any, idx: number) => (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all whitespace-nowrap border ${activeCaseIndex === idx
                                            ...
                                            : `${isDarkMode ? 'bg-zinc-900/50 text-zinc-500 hover:bg-zinc-800' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'} ${borderColor}`
                                            }`}
                                    >
                                        Case {idx + 1}
                                    </button>
                                            ...
                                            <div>
                                                ...
                                                <div className="relative group">
                                                    <textarea
                                                        ...
                                                        className={`w-full border rounded-xl p-4 text-xs font-mono outline-none resize-none min-h-[100px] transition-colors ${inputBg} ${textColor} ${isDarkMode ? 'group-hover:border-white/10' : 'group-hover:border-slate-300'}`}
                                                    />
                                                </div>
                                            </div>
                                            <div>
                                                ...
                                                <div className="relative group">
                                                    <textarea
                                                        ...
                                                        className={`w-full border rounded-xl p-4 text-xs font-mono outline-none resize-none min-h-[80px] transition-colors ${inputBg} ${textColor} ${isDarkMode ? 'group-hover:border-white/10' : 'group-hover:border-slate-300'}`}
                                                    />
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className="h-full flex flex-col p-6 overflow-y-auto custom-scrollbar">
                        {!output ? (
                            <div className="h-full flex flex-col items-center justify-center text-zinc-600">
                                <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 border ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-slate-100 border-slate-200'}`}>
                            ...
                            <>
                                {/* Verdict Header */}
                                <div className={`mb-6 p-5 rounded-xl border relative overflow-hidden shrink-0 ${output.verdict === 'Accepted' ? 'bg-green-500/5 border-green-500/20' :
                                    isPending ? 'bg-blue-500/5 border-blue-500/20' :
                                        'bg-red-500/5 border-red-500/20'
                                    }`}>
                                    <div className={`absolute top-0 left-0 w-1 h-full ${output.verdict === 'Accepted' ? 'bg-green-500' :
                                    ...
                                    <div className="flex items-center justify-between relative z-10">
                                        ...
                                        {output.runtime_ms !== undefined && !isPending && (
                                            <div className="text-right">
                                                <div className="text-[10px] text-zinc-600 uppercase tracking-widest font-bold">Runtime</div>
                                                <div className={`text-sm font-mono font-bold ${textColor}`}>{output.runtime_ms} ms</div>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                {/* Test Cases Summary */}
                                {output.testcases && output.testcases.length > 0 && !isPending ? (
                                    <>
                                        <div className="mb-4">
                                            ...
                                            return (
                                                <div className="space-y-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className={`border p-3 rounded-lg font-mono text-xs whitespace-pre-wrap max-h-32 overflow-y-auto custom-scrollbar ${inputBg} ${textColor}`}>
                                                            {tc.input}
                                                        </div>
                                                    </div>
                                    ...
                                    !isPending && output.error && (
                                        <div className="bg-red-500/5 p-6 rounded-xl border border-red-500/20">
                                            ...
                                            <pre className="whitespace-pre-wrap text-xs font-mono text-red-500/80 leading-relaxed">{output.error}</pre>
                                        </div>
                                    )
                                )}
                            </>
                ...
                {activeTab === "submissions" && (
                    <div className="h-full p-6 overflow-y-auto custom-scrollbar">
                        <SubmissionsList submissions={submissions} onSelectSubmission={onSelectSubmission} isDarkMode={isDarkMode} />
                    </div>
                )}
            </div>
        </div>
...
function SubmissionsList({ submissions, onSelectSubmission, isDarkMode = true }: { submissions?: any[], onSelectSubmission?: (submission: any) => void, isDarkMode?: boolean }) {
    if (!submissions) return <div className={`p-6 text-sm ${isDarkMode ? 'text-zinc-500' : 'text-slate-500'}`}>Loading submissions...</div>;

    if (submissions.length === 0) return (
        <div className={`h-full flex flex-col items-center justify-center p-6 ${isDarkMode ? 'text-zinc-500' : 'text-slate-500'}`}>
            <Clock size={48} className="mb-4 opacity-20" />
            <div className={`text-lg font-medium mb-2 ${isDarkMode ? 'text-zinc-400' : 'text-slate-600'}`}>No Submissions Yet</div>
            <div className="text-sm text-center max-w-[200px] opacity-80">
                Submit your solution to track your progress here.
            </div>
        </div>
    );

    const cardBg = isDarkMode ? "bg-[#121212] border-white/5 hover:bg-[#1a1a1a] hover:border-white/10 hover:shadow-black/20" : "bg-white border-slate-200 hover:bg-slate-50 hover:border-slate-300 hover:shadow-slate-200/50";
    const labelColor = isDarkMode ? "text-zinc-600" : "text-slate-400";
    const valueColor = isDarkMode ? "text-zinc-300" : "text-slate-700";
    const badgeBg = isDarkMode ? "bg-white/5 border-white/5" : "bg-slate-100 border-slate-200 text-slate-700";
            ...
            {submissions.map((sub: any, idx: number) => {
                ...
                return (
                    <div
                        ...
                        className={`group flex flex-col border rounded-xl overflow-hidden transition-all hover:shadow-lg cursor-pointer ${cardBg}`}
                    >
                        <div className="flex items-center justify-between p-4 relative overflow-hidden">
                            ...
                            <div className="flex items-start gap-4 relative z-10">
                                ...
                                <div>
                                    ...
                                    <div className={`text-[10px] mt-1 font-medium ${isDarkMode ? 'text-zinc-500' : 'text-slate-400'}`}>
                            ...
                            <div className="flex items-center gap-6 relative z-10">
                                <div className="text-right">
                                    <div className={`text-[9px] uppercase font-bold tracking-widest mb-0.5 ${labelColor}`}>Lang</div>
                                    <div className={`text-xs font-mono px-2 py-0.5 rounded border capitalize ${badgeBg} ${isDarkMode ? 'text-zinc-300' : ''}`}>
                                        {sub.language}
                                    </div>
                                </div>
                                <div className="text-right min-w-[50px]">
                                    <div className={`text-[9px] uppercase font-bold tracking-widest mb-0.5 ${labelColor}`}>Time</div>
                                    <div className={`text-xs font-mono font-medium ${valueColor}`}>
                                        {sub.runtime_ms !== null ? `${sub.runtime_ms} ms` : '-'}
                                    </div>
                                </div>
                                <div className="text-right min-w-[50px]">
                                    <div className={`text-[9px] uppercase font-bold tracking-widest mb-0.5 ${labelColor}`}>Mem</div>
                                    <div className={`text-xs font-mono font-medium ${valueColor}`}>
